...
% forward propagation 
% normal equation of J

X = [ ones(m,1) X ]; %adding bias;
z2 = Theta1 * X';
a2 = sigmoid(z2);
a2 = [ ones(m,1) a2' ]; %adding bias and sigmoid gives a row vector
z3 = Theta2 * a2' ;
h = sigmoid(z3);

y_i = zeros(num_labels , m);
for i = 1:m
    y_i(y(i),i) = 1; % finding for y(i)
end
...
for t=1:m
    ...
    a1 = X(t,:); % X already have a bias. So, no need to add bias;
    a1 = a1'; % (401*1)
